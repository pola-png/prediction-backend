cking for results for ${matchesToCheck.length} OpenLigaDB matches.`);
...
async function fetchFromSoccersAPI() {
    ...
    if (!SOCCERSAPI_USER || !SOCCERSAPI_TOKEN) throw new Error("SoccersAPI credentials missing");
    ...
    for (const matchData of liveMatches) {
        ...
        if (await Match.findOne({ externalId })) continue;
...
async function fetchAndStoreMatches() {
    let newMatchesCount = 0, updatedMatchesCount = 0, newHistoryCount = 0;
        ...
        try {
            ...
            for (const matchData of history) {
                ...
                await Match.create({
                    ...
                    homeTeam: homeTeam._id,
                    awayTeam: awayTeam._id,
                    homeGoals: matchData.score?.ft?.[0] ?? null,
                    awayGoals: matchData.score?.ft?.[1] ?? null,
                });
                newHistoryCount++;
            }
        } catch {}
...
async function fetchAndStoreResults() {
    const matchesToCheck = await Match.find({ status: 'scheduled', matchDateUtc: { $lt: new Date() }, source: 'openligadb' });
    let updatedCount = 0;
        ...
        try {
            ...
            if (matchResult?.matchIsFinished) {
                ...
                if (homeGoals !== null && awayGoals !== null) {
                    await Match.updateOne({ _id: match._id }, { $set: { status: 'finished', homeGoals, awayGoals, updatedAt: new Date() } });
                    updatedCount++;
                }
            }
        } catch {}
...
module.exports = { fetchAndStoreMatches, generateAllPredictions, fetchAndStoreResults };
const axios = require('axios');
const Match = require('../models/Match');
const Team = require('../models/Team');
const Prediction = require('../models/Prediction');
